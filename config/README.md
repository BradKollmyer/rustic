...
(`s3_aws_glacier_hot_cold.toml`) with `--repo-hot` and rustic
`data_storage_class` (data packs only). Native RestoreObject is
`enable_restore = true`. Do **not** set OpenDAL `default_storage_class` to
`GLACIER` / `DEEP_ARCHIVE` on a single-bucket repository — that archives keys,
snapshots, index, and tree packs as well as data. See the comments in
`s3_aws.toml` and `s3_aws_glacier_hot_cold.toml`.
